...
const google = L.tileLayer(
	'https://mt1.google.com/vt/lyrs=r&x={x}&y={y}&z={z}', {
		attribution: "<a href='https://developers.google.com/maps/documentation'>Google Map</a>"
	});
...
L.easyButton({
	...
	states: [{
		stateName: 'locate',
		icon: '<img src="https://github.com/fujimaru703/fukuko-location/blob/main/icon/location.png" style="width:20px;">',
		title: '現在地に移動',
...
const label290 = ['2007', '8015', '8016', '8037', '8038', '8057', '8058',
	...
	'8127', '8137', '8138', '8139', '8140', '8141', '8143', '8144', '8145',
	'8146', '2006', '0741', '0743', '0744', '0774', '0775', '0803', '8060',
	'8061', '7165', '8080', '80801', '8098', '8099', '8100', '8128', '8129',
	'8156', '8157', '8158', '0887', '0889'
];
...
labelkanachumio.forEach(label => labelIconMap.set(label,
	'icon/kanachu-mio.png'));

const labelkanachumk = ['1623', '1625', '1668', '1706', '1769', '1771', '0562',
	'1860', '1863', '0580', '0581'
];
labelkanachumk.forEach(label => labelIconMap.set(label, 'icon/kanachu-mk.png'));

const labelkanachump35 = ['1920', '7032', '1924', '7034', '1945', '7118',
	'7157', '8066', '8068'
];
labelkanachump35.forEach(label => labelIconMap.set(label,
	'icon/kanachu-mp35.png'));

const labelkanachump37 = ['7024', '7028', '1891', '2001'];
labelkanachump37.forEach(label => labelIconMap.set(label,
	'icon/kanachu-mp37.png'));
...
const labelkeiohr = ['1544', '1638', '1641', '1645', '1646', '1657', '1698',
	'1740', '1764', '7198'
];
labelkeiohr.forEach(label => labelIconMap.set(label, 'icon/keio-hr.png'));

const labelkeiomklong = ['7047', '7050', '7052', '7061', '7064'];
labelkeiomklong.forEach(label => labelIconMap.set(label,
	'icon/keio-mk-long.png'));

const labelklhr = ['7030', '7033', '1943', '7036', '7045', '1974', '2027',
	'7213', '0575'
];
labelklhr.forEach(label => labelIconMap.set(label, 'icon/kl-hr.png'));

const labelkllt = ['1723', '7072', '1812', '1822', '1828', '1843', '8020',
	'8032', '8043'
];
...
const labelmk = ['0044', '0672', '0864', '2009', '2022', '8019', '8022',
	'8073', '8076', '7199', '1541'
];
...
const labelmp35silver = ['7106', '7109', '7111', '7112', '7114', '7117',
	'7119'
];
labelmp35silver.forEach(label => labelIconMap.set(label,
	'icon/mp35-silver.png'));

const labelmp37 = ['2039', '2046', '8006', '7058', '8007', '8009', '8010',
	'8012', '7078', '7079', '7084', '7088', '7089', '7090', '7091', '7098',
	'7099', '7100', '7129', '7130', '7136', '7139', '7143', '7145', '7151',
	'7152', '8096'
];
...
labelmp37silver.forEach(label => labelIconMap.set(label,
	'icon/mp37-silver.png'));
...
const labelpdgkr = ['1836', '1837', '0997', '0998', '1091', '1092', '1196',
	'1197', '1217', '7132', '7148', '7150', '7153', '8112', '8069', '0885'
];
labelpdgkr.forEach(label => labelIconMap.set(label, 'icon/pdg-kr.png'));

const labelpdglr = ['0995', '0996', '8088', '1089', '1090', '1192', '1193',
	'1194', '1219', '1220', '1221', '0997', '0998', '1091', '1092', '1196',
	'1197', '1217', '1605', '1835', '8036', '8041'
];
...
const labelrakuraku = ['0017', '0018', '0867', '0021', '0847', '8163', '8162',
	'0116', '0117', '0119', '0120', '0121', '0191', '0193', '0236', '0237',
	'0238', '2045', '0299', '0302', '0397', '0408', '0406', '0407', '0409',
	'8134', '8135'
];
...
const labelrinkomio = ['1336', '1369', '1379', '0433', '0451', '1710', '1720',
	'2020', '1732', '1850', '1854', '1857', '2041', '8150', '8025'
];
...
const labeltkgmk = ['1472', '1473', '0879', '0510', '0878', '8045', '1660',
	'0878', '7133', '7134', '1833', '1834', '1926', '1927', '1928', '1929',
	'1930', '2004', '2005'
];
labeltkgmk.forEach(label => labelIconMap.set(label, 'icon/tkg-mk.png'));

const labeltoeimklong = ['7046', '1990', '7047', '1999'];
labeltoeimklong.forEach(label => labelIconMap.set(label,
	'icon/toei-mk-long.png'));

const label234 = ['8168', '8167', '0429', '0430', '0431', '8039', '7135',
	'7144', '8159', '7163', '8072', '7164', '7166', '8086', '7171', '8089',
	'8090', '8085', '7172', '8097', '7181', '8103', '7182', '8107', '7186',
	'8113', '7187', '8126', '8133'
];
label234.forEach(label => labelIconMap.set(label, 'icon/234.png'));

function createBusIcon(label, zoom) {
	const size = Math.max(32, Math.min(48, Math.round(32 * Math.pow(1.2, zoom -
		13))));
...
async function loadRoutes() {
	const res = await fetch(
		'https://raw.githubusercontent.com/fujimaru703/fukuko-location/main/routes.txt'
	);
	...
	for (const line of lines) {
		...
		routeNames[cols[idIdx]] = cols[nameIdx];
	}
}

async function loadTrips() {
	const res = await fetch(
		'https://raw.githubusercontent.com/fujimaru703/fukuko-location/main/trips.txt'
	);
	const text = await res.text();
	const [header, ...lines] = text.trim().split('\n');
	const headers = header.split(',');
	const idIdx = headers.indexOf('trip_id');
	const headIdx = headers.indexOf('trip_headsign');
...
async function loadShapes() {
	const res = await fetch(
		"https://raw.githubusercontent.com/fujimaru703/fukuko-location/main/shapes.txt"
	);
	const text = await res.text();
	...
	for (const id in shapePoints) {
		const sorted = shapePoints[id].sort((a, b) => a.seq - b.seq);
		shapeMap[id] = sorted.map(p => [p.lat, p.lon]);
	}
}
...
async function loadStopTimes() {
	const res = await fetch(
		'https://raw.githubusercontent.com/fujimaru703/fukuko-location/main/stop_times.txt'
	);
	const text = await res.text();
	const [header, ...lines] = text.trim().split('\n');
	const headers = header.split(',');
...
async function loadStops() {
	const res = await fetch(
		'https://raw.githubusercontent.com/fujimaru703/fukuko-location/main/stops.txt'
	);
	const text = await res.text();
	const [header, ...lines] = text.trim().split('\n');
	const headers = header.split(',');
...
async function loadDelays() {
	...
	for (const entity of data.entity) {
		...
		if (trip && updates ? .length > 0) {
			...
			tripDelays[trip.tripId] = delayMap;
		}
	}
}
...
async function showDelayedBusHeatmap() {
	...
	for (const entity of data.entity) {
		...
		if (pos) {
			const icon = createBusIcon(label, zoom);
			...
			marker.on('click', () => {
				...
				const shapeCoords = shapeMap[shapeId];
				...
				const outerLine = L.polyline(shapeCoords, {
					color: "white",
					weight: 8,
					weight: 12,
					opacity: 1
				...
				const innerLine = L.polyline(shapeCoords, {
					color: "dodgerblue",
					weight: 4,
					weight: 6,
					opacity: 1
				...
				const futureStops = Object.entries(updates)
					.filter(([s, info]) => parseInt(s) > seq)
					.map(([s, info]) => info.stopId)
					.filter(id => !!id);

				for (const [seqStr, info] of Object.entries(updates)) {
					const seqNum = parseInt(seqStr);
					if (seqNum <= seq) continue;

					const stopId = info.stopId;
					const stop = stopDetails[stopId];
					const name = stopNames[stopId];
					if (!stop) continue;


					let unixTime = null;
					const stopTimesForTrip = scheduledTimes[tripId];
					...
					const labelText = `${name}（${arrivalText}）`;
			...
			const delayText = formatDelay(delaySec || 0);
			const routeName = routeNames[routeId] || '路線名不明';
			const headsign = tripHeadsigns[tripId] || '行先不明';
	...
	if (heatmapVisible && delayPoints.length > 0) {
		heatLayer.addTo(map);
	}
}
